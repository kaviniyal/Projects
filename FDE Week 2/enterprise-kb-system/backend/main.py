...
Phase 2: ETL Pipeline + Extended Analytics.
"""
...
# Ensure the ETL package (project root) is importable from the backend process
...
# Create database tables (including Phase 2 tables: search_logs, etl_jobs)
...
app = FastAPI(
    ...
    description=(
        "Phase 2 REST API — Enterprise Knowledge Base with ETL Pipeline & Analytics. "
        "Features: authentication, RBAC, articles with approval workflow, "
        "categories, tags, file attachments, comments, ratings, bookmarks, "
        "full-text search with keyword analytics, ETL pipeline management, "
...
app.include_router(etl_router.router)   # Phase 2 ETL management
