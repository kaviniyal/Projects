...
def search_articles(
    ...
    """Full-text search across title, content, summary. Logs keywords for analytics."""
    query = db.query(models.Article).options(
        joinedload(models.Article.category),
        joinedload(models.Article.author),
        joinedload(models.Article.tags),
    ...
    # --- Phase 2: Log search keyword for analytics ---
    if q and q.strip():
        try:
            log = models.SearchLog(
                keyword=q.strip().lower()[:255],
            ...
            db.rollback()  # Never fail the search due to logging errors
