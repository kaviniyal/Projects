...
# ---------------------------------------------------------------------------
# SearchLog — Phase 2: keyword analytics
# ---------------------------------------------------------------------------
class SearchLog(Base):
    """
    Records every search query submitted through the knowledge base so that
    keyword-frequency analytics can be derived in Phase 2.
    """
    ...
    user_id = Column(Integer, ForeignKey("users.user_id"), nullable=True)
    result_count = Column(Integer, nullable=True)   # how many results were returned
    searched_at = Column(DateTime, default=datetime.utcnow, nullable=False, index=True)


# ---------------------------------------------------------------------------
# ETLJob — Phase 2: track ETL pipeline runs
# ---------------------------------------------------------------------------
class ETLJob(Base):
    """
    Tracks every execution of the ETL pipeline so admins can monitor
    import history and diagnose failures.
    """

    __tablename__ = "etl_jobs"

    job_id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    # Status values: queued, running, completed, completed_with_errors, failed
    status = Column(String(30), default="queued", nullable=False, index=True)
    datasets_dir = Column(String(500), nullable=True)
    started_at = Column(DateTime, nullable=True)
    finished_at = Column(DateTime, nullable=True)

    # Row-level counters populated by the load stage
    rows_extracted = Column(Integer, default=0, nullable=False)
    ...
    rows_errored = Column(Integer, default=0, nullable=False)
