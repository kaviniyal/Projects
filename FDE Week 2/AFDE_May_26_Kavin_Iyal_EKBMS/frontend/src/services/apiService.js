...
// ---------------------------------------------------------------------------
// Analytics (Phase 2 extended)
// ---------------------------------------------------------------------------
export const analyticsService = {
  dashboard: async () => (await api.get('/analytics/dashboard')).data,
  full: async () => (await api.get('/analytics')).data,
  // Phase 2 dedicated endpoints
  categoryTrends: async () => (await api.get('/analytics/category-trends')).data,
  searchKeywords: async (limit = 30) => (await api.get('/analytics/search-keywords', { params: { limit } })).data,
  authorActivity: async () => (await api.get('/analytics/author-activity')).data,
  monthlyPublish: async () => (await api.get('/analytics/monthly-publish')).data,
  topTags: async (limit = 20) => (await api.get('/analytics/top-tags', { params: { limit } })).data,
  etlSummary: async () => (await api.get('/analytics/etl-summary')).data,
}

// ---------------------------------------------------------------------------
// ETL Pipeline (Phase 2 — Admin only)
// ---------------------------------------------------------------------------
export const etlService = {
  run: async () => (await api.post('/etl/run')).data,
  listJobs: async (limit = 20) => (await api.get('/etl/jobs', { params: { limit } })).data,
  getJob: async (jobId) => (await api.get(`/etl/jobs/${jobId}`)).data,
  generateReports: async () => (await api.post('/etl/reports')).data,
}
